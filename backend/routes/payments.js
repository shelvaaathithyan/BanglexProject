...
const generateOrderNumber = async () => {
  ...
  const uniqueId = require('crypto').randomBytes(3).toString('hex').toUpperCase();
    ...
    if (createdReservations.length > 0) {
      for (const resv of createdReservations) {
        try {
          ...
          console.error('Rollback Release error:', e);
        }
      }
    }
    await session.abortTransaction();
...
router.post('/release-reservation', async (req, res) => {
  try {
    ...
    const order = await Order.findById(orderId).populate('items.product');
    if (!order) return res.status(404).json({ message: 'Order not found' });
    console.log('Releasing reservations for order', orderId, 'Reservation IDs:', order.reservationIds);
